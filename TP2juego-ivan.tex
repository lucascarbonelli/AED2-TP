...
\begin{Interfaz}
  ...
  \InterfazFuncion{crearMapa}{\In{m}{mapa}}{Juego}%
  {$ mapa(res) = m \land jugadores(res) = \phi \land expulsados(res) = \phi \land posConPokémons(res) = \phi $ }
  [$\Theta(1)$]
  [genera una instancia inicial del juego.]

  \InterfazFuncion{agregarPokemon}{\In{p}{Pokemon}, \In{c}{Coordenada}, \Inout{j}{Juego}}{}
  [$puedoAgregarPokemon(c,j)$]
  {$Ag(c, posConPokemons(j))$}
  [$\bigcirc (|P| + |EC|*log(|EC|))$]
  [agrega un pokemon en una coordenada valida al juego. DESCRIBIR BIEN LA COMPLEJIDAD]

  \InterfazFuncion{agregarJugador}{\Inout{j}{Juego)}}{}
  {$ Ag(ProxID(j), jugadores(j))$}
  [$\bigcirc (\# jugadores(j))$]
  [agrega un nuevo jugador, designandole la proxima ID disponible.]

  \InterfazFuncion{pudoAgregarPokemon}{\In{c}{Coordenada}, \Inout{j}{Juego}}{bool}
  {$res = c \not \in posConPokemons(j)$}
  [$\Theta(1)$]
  [devuelve verdadero si la coordenada pasada por parametro no está en el juego.]

  \InterfazFuncion{debeSancionarse}{\In{e}{Jugador}, \In{c}{Coordenada}, \Inout{j}{Juego}}{bool}
  {$ res = \lnot hayCamino(posicion(e,j),c,mapa(j)) \lor distEuclidea(posicion(e,j),c,mapa(j)) > 100$}
  [$\Theta(1)$]
  [devuelve verdadero si el jugador realizó un movimiento invalido.]
  
  \InterfazFuncion{debeExpulsarse}{\In{e}{Jugador}, \In{c}{Coordenada}, \Inout{j}{Juego}}{bool}
  [$e \in jugadores(j)$]
  {$res = 10 == sanciones(e,j)$}
  [$\Theta(1)$]
  [devuelve verdadero si las sanciones del jugador pasado son 10.]

  \InterfazFuncion{hayPokemonCercano}{\In{c}{Coordenada}, \In{j}{Juego}}{bool}
  {$res = \lnot \phi ? (buscarPokemonsCercano(c,posConPokemons(j),j))$}
  [$\Theta(1)$]
  [devuelve verdadero si hay algún pokemon cercano a la coordenada pasada.]
  
  \InterfazFuncion{posPokemonCercano}{\In{c}{Coordenada}, \In{j}{Juego}}{Coordenada}
  [$hayPokemonCercano(c,j)$]
  {$res = dameUno(buscarPokemonsCercano(c,posConPokemons(j),j))$}
  [$\Theta(1)$]
  [devuelve una coordenada, cercana a la coordenada pasada, donde hay un pokemon.]
    
  \InterfazFuncion{entrenadoresPosibles}{\In{c}{Coordenada}, \In{es}{conj(jugador)}, \In{j}{Juego}}{conj(jugador)}
  [$hayPokemonCercano(c,j) \land es \subseteq jugadoresConcectados(j)$]
  {$res = entrenadoresPosibles(c,es,j)$}
  [$\Theta(1)$]
  [devuelve un conjunto con los jugadores cercanos a un pokemon.]
