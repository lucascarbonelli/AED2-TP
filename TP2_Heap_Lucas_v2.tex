...
\begin{algorithm}[H]{\textbf{iDesencolar}(\Inout{c}{heapmod})}
		...
		\State $puntero(nodo) \ buscarUlt \gets c.tope$ \Comment {Se crea un puntero para buscar el último nodo} // $\Theta(1)$
			...
			\Statex \underline{Justificación: } El algoritmo cuenta con un ciclo que se repetirá $log(\#nodos(c)$ (recorre el árbol de arriba a abajo), llama a dos funciones con costo $\Theta(log(\#nodos(c))$ y el resto de las operaciones tienen costo $\Theta(1)$
...
\begin{algorithm}[H]{\textbf{iSiftDown}(\Inout{c}{heapmod}, \In{p}{puntero(nodo)})}
    	...
    	\If{$p \neq swap$} \Comment {Vemos si es necesario reacomodar los nodos} $\Theta(1)$
	    	\State {$jugadorHeap \ mayorPrior \gets (*swap).elemento$} \Comment {Creamos una variable para intercambiar} $\Theta(1)$
	    	\State {$(*swap).elemento \gets (*p).elemento$} \Comment {Intercambiamos el padre con el hijo} $\Theta(1)$
	    	\State {$(*p).elemento \gets mayorPrior$} \Comment {} $\Theta(1)$
...
\begin{algorithm}[H]{\textbf{iSiftUp}(\Inout{c}{heapmod}, \In{p}{puntero(nodo)})}
    	...
    	\EndIf
    	\If{$p \neq swap$} \Comment {Vemos si es necesario reacomodar los nodos} $\Theta(1)$
	    	\State {$jugadorHeap \ menorPrior \gets (*swap).elemento$} \Comment {Creamos una variable para intercambio} $\Theta(1)$
	    	\State {$(*swap).elemento \gets (*p).elemento$} \Comment {Intercambiamos el padre con el hijo} $\Theta(1)$
	    	\State {$(*p).elemento \gets menorPrior$} \Comment {} $\Theta(1)$
...
\begin{algorithm}[H]{\textbf{iEliminarSiguiente}(\In{it}{itHeapmod}) $\to$ $res$ : jugadorHeap}
    	\begin{algorithmic}[1]
		\State $puntero(nodo) \ buscarUlt \gets (*it.heap).tope$ \Comment {Se crea un puntero para buscar el último nodo} // $\Theta(1)$
    		\If{$(*buscarUlt).hijoIzq = NULL \land (*buscarUlt).hijoDer = NULL$} \Comment $\Theta(1)$
			...
			\While{$\neg((*buscarUlt).hijoIzq = NULL \land (*buscarUlt).hijoDer = NULL)$} \Comment {} // $\Theta(1)$
				...
				\EndIf
			\EndWhile
			\State $(*it.siguiente).elemeto \gets (*buscarUlt).elemento$ \Comment {Si elimina el tope colocando el último elemento} // $\Theta(1)$
			\State $puntero(nodo) \ padreUlt \gets (*buscarUlt).padre$ \Comment {Se crea un puntero al padre del nodo a eliminar} // $\Theta(1)$
			\If{$(*padreUlt).hijoDer = buscarUlt$} \Comment {Vemos si el nodo a eliminar es el derecho} // $\Theta(1)$
				\State $(*padreUlt).hijoDer \gets NULL$ \Comment {Eliminamos el nodo} // $\Theta(1)$
			\Else
			...
			\EndIf
			\State $iCorregirProfundidad(padreUlt, c)$ \Comment {Corregimos la profundidad} // $\Theta(log(\#nodos(c))$
			\State $iSiftDown(it.siguiente, c) $ \Comment {Se repara el heap} // $\Theta(log(\#nodos(c))$
			\State $iSiftUp(it.siguiente, c) $ \Comment {Se repara el heap} // $\Theta(log(\#nodos(c))$
			\EndIf
    	
			\medskip
			\Statex \underline{Complejidad:} $\Theta(log(\#nodos(c))$
			\Statex \underline{Justificación:} El algoritmo cuenta con un ciclo que se repetirá $log(\#nodos(c)$ (recorre el árbol de arriba a abajo), llama a tres funciones con costo $\Theta(log(\#nodos(c))$ y el resto de las operaciones tienen costo $\Theta(1)$
    	\end{algorithmic}
\end{algorithm}
