...
Este módulo implementa una cola de prioridad que permite eliminar elemntos en tiempo logaritmico. La cola de prioridad está formada por un conjunto de nodos, los cuales contienen el elemento que se quiere colocar en la cola y un número que define univocamente la prioridad del elemento respecto del resto de los elemento en la cola. Siendo n el número asociado a un elemento dado, debe cumplirse que los elementos con asociados a los números 2*n+1 y 2*n+2 tienen menor prioridad que el elemento asociado a n, y el elemento asociado a (n-1)/2 tiene mayor prioridad.
...
%  {c $\igobs$ encolar($c_0$) $\land$ $\Pi_{2}$(res) $\igobs$ a $\land$ $\Pi_{1}$(res) $<$ $\#$ heapSecu(c) $\land$ \\($\Pi_{1}$(res) $=$ 0 $\lor$ (($\exists$ x $\in$ heapSecu(c)) $\Pi_{1}$(x) = ($\Pi_{1}(res)-1)/2$ $\land$ $\Pi_{2}$(res) $<$ $\Pi_{2}$(x))) $\land$ \\($\Pi_{1}$(res) $>$ $\#$heapSecu(c)/2 $\lor$ (($\exists$ x,y $\in$ heapSecu(c)) \\$\Pi_{1}$(x) = (2*$\Pi_{1}(res)$)+1 $\land$ $\Pi_{2}$(x) $<$ $\Pi_{2}$(res) $\land$ \\$\Pi_{1}$(y) = (2*$\Pi_{1}(res)$)+2 $\land$ $\Pi_{2}$(y) $<$ $\Pi_{2}$(res)))}
  ...
  \begin{tad}{Cola de Prioridad extendida}
    ...
    \tadAlinearFunciones{eliminar(c, a)}{heapMod($\alpha$), $\alpha$}
    \tadOperacion{eliminar}{heapMod($\alpha$), $\alpha$}{heapMod($\alpha$)}{}
    \tadOperacion{heapSecu}{heapMod($\alpha$)}{secu($\alpha$)}{}
    
    \tadAxiomas
    \tadAlinearAxiomas{eliminar(c, a)}
    \tadAxioma{eliminar(c, a)}{HACER!!}
    \tadAxioma{heapSecu(c)}{HACER!!}
  \end{tad}
...
\begin{Representacion}
  ...
  En este módulo vamos a utilizar un vector para representar la cola, donde cada elemento es un puntero a una tupla (nodo). La primera componente del nodo es la posicion de la tupla en el vector, mientras que la segunda componente es el elemento que queremos colocar en la cola de prioridad.
  ...
  \Rep[vector( puntero(nodo))][c]{($\forall i$: nat) ($0 \leq i <$ Longitud(c)) $\impluego$ c[i].posicion = i $\land$\\($\forall i$: nat) ($0 < i <$ Longitud(c)) $\impluego$ $c[i].dato < c[(i-1)/2].dato$ $\land$\\ ($\forall i$: nat) $\impluego$ ($0 \leq i <$ Longitud(c)/2) $c[(2*i)+1].dato < c[i].dato \land c[2*i)+2].dato < c[i].dato$)}
...
\begin{algorithm}[H]{\textbf{iEncolar}(\In{a}{$\alpha$}, \Inout{c}{vector( puntero(nodo))}) $\to$ $res$ : $nodo$}
    	...
    	\State $puntero(nodo) \ nuevo \gets \& \langle iLongitud(c), a \rangle$ \Comment {Se crea un nuevo nodo, $a$ se pasa por referencia} // $\Theta(1)$
  		...
  		\State $iSiftUp(c, iLongitud(c)-1) $ \Comment {Se repara el heap} // $\Theta(log(iLongitud(c)))$
			...
			\medskip
			\Statex \underline{Complejidad:} $\Theta(log(iLongitud(c)) + ($f$(long($c$)) $+$ $copy(nuevo)$))$
			\Statex \underline{Justificación:} 
    	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]{\textbf{iSiftDown}(\Inout{c}{vector( puntero(nodo)), \In{i}{nat})}}
    	...
    	\If{$l < iLongitud(c) \land c[l].dato > c[i].dato$} \Comment {Vemos si el dato del hijo izquierdo es el mayor} $\Theta(1)$
    		\State {$largest = l$}	\Comment {El hijo izquierdo es mayor que el padre, guardamos la posicion} $\Theta(1)$
			\EndIf
    	\If{$r < iLongitud(c) \land c[r].dato > c[largest].dato$} \Comment {Vemos si el dato del hijo derecho es el mayor} $\Theta(1)$
    	...
    	\If{$largest \neq i$} \Comment {Algun hijo es mayor} $\Theta(1)$
    		\State {$puntero(nodo) \ nuevo = c[i]$}  \Comment {Creamos una variable para intercambiar los elementos} $\Theta(1)$
...
\begin{algorithm}[H]{\textbf{iSiftUp}(\Inout{c}{vector( puntero(nodo)), \In{i}{nat})}}
    	...
    	\If{$i \neq 0 \land c[(i-1)/2].dato < c[(i].dato$} \Comment {Vemos que no esta al tope de la cola ni es menor al padres} $\Theta(1)$
    		\State {$puntero(nodo) \ nuevo = c[i]$} \Comment {Creamos una variable para intercambiar los elementos} $\Theta(1)$
    		\State {$c[i] \gets c[(i-1)/2]$} \Comment {Intercambiamos el padre con el hijo} $\Theta(1)$
    		\State {$c[(i-1)/2] \gets nuevo$} \Comment $\Theta(1)$
    		\State {$iSiftUp(c, largest)$} \Comment {Llamamos a la funcion recursivamente} $\Theta(1)$
			\EndIf
