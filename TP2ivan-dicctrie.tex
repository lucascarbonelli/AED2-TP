...
\begin{Representacion}
  ...
  %Una parte del enunciado del TP dice que se pueden escribir en castellano aquellas partes del rep que tengan punteros. De ultima, dejo escrito esto en castellano.
...
\begin{algorithm}[H]{\textbf{iCopiar}(\In{d}{dic})$\to$ $res$ : dicc($string , \sigma$)}	
	\begin{algorithmic}[1]
			\State $res.dNodo \gets NULL$ \Comment $\Theta(1)$
			\State $res.claves \gets iVacío()$ \Comment $\Theta(1)$
			 \State $claves \gets iClaves(d)$ \Comment $\Theta(1)$
			 \While{$\lnot iEsVacío?(claves)$} \Comment $\Theta (1)$
