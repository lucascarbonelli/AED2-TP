...
\begin{algorithm}[H]{\textbf{iBorrar}(\Inout{d}{dicc}, \In{c}{$string$})}
	\begin{algorithmic}[1]
			 \State $ruptura \gets d.dNodo$ \Comment $\Theta (1)$
			 \State $letraRuptura \gets buscarRuptura(d.dNodo, c, ruptura)$ \Comment $\mathcal{O} (iLongitud(c))$
			 \State $nodoDef \gets buscarNodoDef(dNodo, c)$ \Comment $\mathcal{O} (iLongitud(c))$
			 \If{$d.dNodo = ruptura$} \Comment $\Theta (1)$
			 	\If{$cantidadNoNull(nodoDef) = 0$} \Comment $\Theta (1)$
			 		\If{$cantidadNoNull(d.dNodo) = 0$} \Comment $\Theta (1)$
