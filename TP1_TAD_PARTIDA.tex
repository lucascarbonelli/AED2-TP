...
\tadOperacion{movOK}{jugador, jugador, partida, coordenada}{bool}{}
...
\tadAxioma{pokemonesDeJugador(mover(p, j', c),j)}{\IF j$\neq$j' $\land$ hayAlgunPoke(j, coordenadasTotalesPoke(pokemonesSalvajes(p), p)) $\yluego$ movOK(j, j', p, c) $\yluego$ pokeParaCapturar(j, p) $\yluego$ esElElegido(j, p) THEN Ag(esePokemon(j, p), pokemonesDeJugador) ELSE pokemonesDeJugador(j, p) FI }
...
\tadAxioma{movOK(j, j', p, c)}{$\lnot$ enRango(posicionJugador(j'), coordPoke(j,\\ coordenadasTotalesPoke(pokemonesSalvajes(p), p)), 2) $\land$ $\lnot$ enRango(c, coordenadasTotalesPoke(pokemonesSalvajes(p), p), 2))}
