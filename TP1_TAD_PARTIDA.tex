...
\tadOperacion{hayPokeEnRango}{jugador, conj(coordenada)}{bool}{}
...
\tadAxioma{div}{nat, nat}{nat}
\tadAxioma{mod}{nat, nat}{nat}
...
\tadAxioma{hayPokeEnRango(j, C)}{\IF $\emptyset$?(C) THEN False ELSE {\IF enRango(posicionJugador(j), dameUno(C), 2) THEN True ELSE hayPokeEnRango(j,sinUno(C))FI} FI}
